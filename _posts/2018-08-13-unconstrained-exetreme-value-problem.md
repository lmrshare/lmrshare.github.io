...
* [共轭梯度法(Conjugate Gradient---CG)](#Conjugate-Direction)
* [Ref](#will-search)


&emsp;&emsp;本文是关于求解的, 主要介绍无约束极值问题的一些常用方法, 同时也是是CNN这篇文章中Hessian Free Optimization的补充材料. 本文来源于教材<< ___运筹学___ >>中的 ___无约束极值问题的解法___ 一节, 这篇笔记的特点在于: 对书中一些公式细节进行补充说明、提出作者自己的推导方法, 此外, 给出实际案例以及代码来加深理解.
...
&emsp;&emsp;首先, 我先交代通过该分析可以得到的结论: 在$ X^k $ 的附近 __充分小__ 的距离内, 只要$ P^k $与$ X^k $的梯度$ \nabla f(X^k) $的夹角大于$ 90^o $, 那么向该方向向量行走就可以改善结果, 换句话说可以使目标函数进一步朝极小值迈进. 具体的分析如下:

原函数$ f(X) $的一阶泰勒展开如下为$ f(X) = f(X^k + \lambda P^k) = f(X^k) + \lambda \nabla f(X^k)^T P^k + o(\lambda) $
...
: [xxxxxx](http://andrew.gibiansky.com/blog/machine-learning/hessian-free-optimization/)
...
### <a name="Conjugate-Gradient"></a>法共轭梯度法(Conjugate Gradient---CGD
...
2017 A Deep Cascade of Convolutional Neural Networks for MR Image Reconstruction:

- [0. 医学图像重建的一篇博文](http://leotam.github.io/general/2017/12/25/DLtools.html)
- [1. source code of author](https://github.com/js3611/Deep-MRI-Reconstruction)

2017 CNNbasedMedicalSegmentation:

- [1. source code of author](https://github.com/BRML/CNNbasedMedicalSegmentation)
